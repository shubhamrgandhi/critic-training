...
class DefaultAgent:
    ...
    def query(self) -> dict:
        ...
        # mistral-only cleanup: drop extra keys that vLLM's Mistral template rejects
        model_name = (
            getattr(self.model, "model_name", None)
            or getattr(self.model, "name", None)
            or (self.model.get_template_vars().get("model_name") if hasattr(self.model, "get_template_vars") else "")
            or str(self.model)
        )
        ...
        if "mistral" in str(model_name).lower():
            response = self.model.query(send_messages, stop=["</s>"])
        else:
            response = self.model.query(send_messages)
