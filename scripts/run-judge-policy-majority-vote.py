...
- If a command reads a very broad span (>200 lines) for a targeted need → redundant.
...
1) FILE-VIEW MAP: For each file, maintain merged viewed line-intervals.
   - A read is a duplicate if >=30% of its requested span has been seen previously.
...
3) Else check REDUNDANT policies (R1-R12). If any applies → REDUNDANT (cite the rule).
4) Else → ESSENTIAL (default).

=====================================================
D) ESSENTIAL STEP POLICIES
=====================================================
E1. FIRST-TIME, TARGETED, EFFICIENT INFORMATION ACQUISITION
- Retrieves information not previously seen AND does so with targeted scope AND does it in an efficient manner.
  Examples: first search for symbols/paths; first read of an unseen line-range (not whole-file; use line ranges).
  Tests:
    • If >=30% overlap with previously viewed lines → NOT E1 (see R1).
...
- Runs tests/builds/programs to observe effects of a prior material (behavior-affecting) change.
  Notes:
    • Re-running the *same* ad-hoc validation with no new changes is NOT E3 (see R12).
    • Splitting tiny validations across many steps when one comprehensive run is possible is NOT E3 for the extra runs (see R10/R11/R12).

E4. FINAL SUBMISSION (UNIQUE, TASK-DEFINED)
- The final submission command is ALWAYS essential.

=====================================================
E) REDUNDANT STEP POLICIES
=====================================================
(When redundant, include the policy code(s) in violated_policies and select EXACTLY ONE category from Section F.)

1) INFORMATION REDUNDANCY
R1. DUPLICATE READ  (category: duplicate_read)
- Re-reading content already retrieved earlier (>=30% line-span overlap) without external changes.
- Reading after the agent's own write is always redundant (GC2), even for small/local edits.

R2. DUPLICATE SEARCH  (category: duplicate_search)
- Repeating identical or substantially similar searches (same intent/pattern/targets) without new parameters or new information.

R3. DUPLICATE STATUS CHECK  (category: duplicate_status_check)
...
- Any whole-file read (hard ban; GC1) OR very broad reads (>200 lines) when hunting a specific target.
  Correction: search (e.g. `grep -n`, `git grep`, ripgrep) → then narrow line-ranged read (e.g., `sed -n 'L-20,L+20p'`).

R5. INEFFICIENT SEARCH STRATEGY  (category: inefficient_search_strategy)
- Issuing multiple near-identical searches in separate steps that could be **combined into one** efficient command, or
- Narrowing in tiny increments where a single well-formed search would answer the question.
  Examples:
    • Grepping the same string in N files over N steps instead of one grep across those files.
    • Sequential `grep -n "X" fileA`, then `grep -n "X" fileB` instead of `grep -n "X" fileA fileB`.
  Correction: combine (single multi-target search), or modify (use alternations/patterns).

R6. UNFOCUSED BROWSING  (category: unfocused_browsing)
- Broad listings/browse passes (e.g., repeated `ls -la`, directory dumps) without filters/patterns that narrow to the target.
  Correction: use `find`/`grep -R` with constraints.

R7. TANGENTIAL EXPLORATION  (category: tangential_exploration)
- Scope drift to actions not supported by current evidence or contrary to recent negative evidence.
  Examples:
    • After codebase-wide searches show **no** occurrences for a symbol, reading unrelated tails/blocks “just to check”.
    • Inspecting distant files/regions without a plausible link to the task after establishing the relevant locus.
  Correction: skip or redirect to evidence-backed targets.

3) FAILED STRATEGY PERSISTENCE
R8. REPEATED FAILED COMMAND  (category: repeated_failed_command)
- Re-running a failing command unchanged (same args/paths/env) without addressing the cause.

R9. CIRCULAR DEBUGGING  (category: circular_debugging)
- Cycling the same ineffective debugging/fix approach more than one time without significant adjustment.

4) UNNECESSARY VERIFICATION
R10. OBVIOUS CONFIRMATION  (category: obvious_confirmation)
- Verifying states that are logically certain from prior successful outputs.
  Examples: checking file existence immediately after a successful create; `git diff` right after `git status` shows no changes.

R11. EXCESSIVE INTERMEDIATE TESTING  (category: excessive_intermediate_testing)
- Running tests/builds with no material change since the last run, or
- Splitting logically-related validations into many small runs rather than one comprehensive run.
  (Catches steps that test small cases individually—e.g., separate Python one-liners for list-of-arrays, empty-list error, and default arrays=None—when a single consolidated test would suffice.)
  Correction: combine into one script and run once.

R12. REDUNDANT SUCCESS VALIDATION  (category: redundant_success_validation)
- Re-running the **same** validation that already passed with no intervening implementation change.
  (Catches re-executing the same reproduction snippet again after a prior success and no code edits.)
  Correction: skip.

=====================================================
F) CATEGORY - POLICY MAPPING
=====================================================
1. Information Redundancy
   - duplicate_read → R1
   - duplicate_search → R2
...
2. Inefficient Information Access
   - overly_broad_read → R4
   - inefficient_search_strategy → R5
   - unfocused_browsing → R6
   - tangential_exploration → R7
...
4. Unnecessary Verification
   ...
   - excessive_intermediate_testing → R11
   - redundant_success_validation → R12
...

{
  ...
  "reasoning": "<2-4 sentences tied to E# or R#. It should be detailed.>",
...
def call_judge(trajectory_content: str, model: str = "gpt-5-mini-2025-08-07", api_key: str = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            # Print cleaner error without verbose output
            print(f"[ERROR] Judge call failed: {type(e).__name__}: {str(e)}")
...
def process_single_trajectory(traj_file: Path, output_dir: Path, model: str, api_key: str, k: int) -> Dict[str, Any]:
    ...
    if not trajectory:
        print(f"[{traj_file}] No trajectory loaded")
        return None
    
    # Extract steps
    steps = extract_steps_from_trajectory(trajectory)
    if not steps:
        print(f"[{traj_file}] No steps extracted")
        return None
    ...
    if os.path.exists(result_file):
        print(f"[{instance_id}] Judge result JSON file already exists. Skipping...")
