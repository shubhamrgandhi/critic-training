#!/usr/bin/env python3
"""
Compute stats for full SWE-Bench Verified (500 instances), organized into
three clean, readable tables, matching the format of get_stats_mini50_v2.py.

Only runs that have a full500 Docker eval (report.json + 500 preds) are included.

  1. full500_baselines.csv  — no-PRM runs
  2. full500_opus_prm.csv   — Claude Opus PRM runs
  3. full500_sft_prm.csv    — Qwen 8B SFT PRM runs

Usage:
    python get_stats_full500_v2.py
"""
import argparse
import hashlib
import json
...
MODEL_PRICING = {
    ...
    "facebook/cwm-sft":                                                                             {"input": 8e-8, "output": 2.8e-7},
    ...
    "shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-instructions-k10-opus-distill-32k-lr5e6-multiturn":{"input": 5e-8, "output": 2e-7},
    "shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-k5-opus-distill-32k-lr5e6-multiturn":              {"input": 5e-8, "output": 2e-7},
}
...
def recompute_prm_cost(traj):
    ...
    if any(entry.get("usage") for entry in feedback_log):
        total_cost = 0.0
...
def compute_stats(run_dir, instance_ids, report_filename="report.json"):
...
def load_traj(run_dir, instance_id):
    inst_dir = run_dir / instance_id
    if not inst_dir.exists():
        return None
    traj_files = list(inst_dir.glob("*.traj.json"))
...
def compute_fallback_stats(prm_dir, base_dir, instance_ids, report_filename="report.json"):
    prm_resolved = set()
...
def has_full500(run_dir):
    """A run counts as full500 if preds.json has 500 entries AND report.json exists."""
    preds = run_dir / "preds.json"
    report = run_dir / "report.json"
    if not (preds.exists() and report.exists()):
        return False
    try:
        return len(json.load(open(preds))) == 500
    except Exception:
        return False
...
def main():
    ...
    for i in range(5):
        ...
        if not d.exists() or not has_full500(d):
            continue
    ...
    # ──────────────────────────────────────────────────────────────────
    # Tables 2 and 3: PRM runs (split by model type)
    # ──────────────────────────────────────────────────────────────────
    opus_headers = [
    ...
    write_csv(out_dir / "full500_baselines.csv", baseline_headers, baseline_rows)
    write_csv(out_dir / "full500_opus_prm.csv", opus_headers, opus_rows)
    write_csv(out_dir / "full500_sft_prm.csv", sft_headers, sft_rows)
    ...
    print_table(f"TABLE 1: Baselines (no PRM), SWE-Bench Verified ({n})", baseline_headers, baseline_rows)
    print_table(f"TABLE 2: Claude Opus PRM runs, SWE-Bench Verified ({n})", opus_headers, opus_rows)
    print_table(f"TABLE 3: Qwen-8B SFT PRM runs, SWE-Bench Verified ({n})", sft_headers, sft_rows)
