#!/bin/bash
# run_critic_max150_mini.sh
#
# Same as run_critic_max150.sh but defaults to MariusHobbhahn/swe-bench-verified-mini
# subset with no slice limit.
#
# Usage: same as run_critic_max150.sh
#   ./run_critic_max150_mini.sh <setting> <prm_interval> <run> <agent_model> [options]
...
if [ -z "$SETTING" ] || [ -z "$PRM_INTERVAL" ] || [ -z "$RUN" ] || [ -z "$AGENT_MODEL" ]; then
    echo "Usage: ./run_critic_max150_mini.sh <setting> <prm_interval> <run> <agent_model> [options]"
    exit 1
fi
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --no-agent-api-base) NO_AGENT_API_BASE="true"; shift ;;
...
# Default prefix dir to run 0 base directory (always use prefix-based runs)
if [ -z "$PREFIX_DIR" ]; then
    PREFIX_DIR="${SCRIPT_DIR}/../results_singularity_max_${EFFECTIVE_STEPS}_steps_prefix/singularity_edit_obs_final_only_0_${AGENT_MODEL}"
fi
OUTPUT="${SCRIPT_DIR}/../results_singularity_max_${EFFECTIVE_STEPS}_steps_prefix/${FULL_SETTING}_${RUN}_${AGENT_MODEL}_prm_${PRM_NAME}"
...
# When --no-agent-api-base is set, leave the config's api_base alone (e.g. for Bedrock).
if [ "$NO_AGENT_API_BASE" = "true" ]; then
    AGENT_API_BASE=""
fi

# --- PRM handling ---
if [ "$PRM_NAME" = "claude-opus-4-6" ]; then
    if [ "$AGENT_NODE" != "localhost" ] || [ -n "$API_BASE" ] || [ -n "$STEP_LIMIT" ] || [ -n "$PRM_DEDUP" ] || [ -n "$PREFIX_DIR" ]; then
        TEMP_CONFIG=$(mktemp /tmp/swebench_config_XXXXXX.yaml)
        trap "rm -f $TEMP_CONFIG" EXIT
        python3 - "$CONFIG" "$TEMP_CONFIG" "$AGENT_API_BASE" "$STEP_LIMIT" "$PRM_DEDUP" "$PRM_DEDUP_THRESHOLD" "$PREFIX_DIR" <<'PYEOF'
import yaml, sys
base_config, temp_config, agent_api_base, step_limit, prm_dedup, prm_dedup_threshold, prefix_dir = sys.argv[1:]
...
PYEOF
    ...
    case "$PRM_NAME" in
        ...
        qwen3-8b-full-sft-prm-opus-distill-32k-lr5e6-flattened)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-opus-distill-32k-lr5e6-flattened"
            PRM_PORT=8071
            ;;
        qwen3-8b-full-sft-prm-opus-distill-32k-lr5e6-multiturn)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-opus-distill-32k-lr5e6-multiturn"
            PRM_PORT=8071
            ;;
        qwen3-8b-full-sft-prm-r2egym-instructions-k10-opus-distill-32k-lr5e6-flattened)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-instructions-k10-opus-distill-32k-lr5e6-flattened"
            PRM_PORT=8071
            ;;
        qwen3-8b-full-sft-prm-r2egym-instructions-k10-opus-distill-32k-lr5e6-multiturn)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-instructions-k10-opus-distill-32k-lr5e6-multiturn"
            PRM_PORT=8071
            ;;
        qwen3-8b-full-sft-prm-r2egym-k5-opus-distill-32k-lr5e6-multiturn)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-k5-opus-distill-32k-lr5e6-multiturn"
            PRM_PORT=8071
            ;;
        qwen3-8b-full-sft-prm-r2egym-instructions-k5-opus-distill-32k-lr5e6-multiturn)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-instructions-k5-opus-distill-32k-lr5e6-multiturn"
            PRM_PORT=8071
            ;;
        qwen3-8b-full-sft-prm-r2egym-swebench-k5-opus-distill-32k-lr5e6-multiturn)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-swebench-k5-opus-distill-32k-lr5e6-multiturn"
            PRM_PORT=8071
            ;;
        qwen3-8b-full-sft-prm-r2egym-swebench-instructions-k5-opus-distill-32k-lr5e6-multiturn)
            PRM_MODEL_NAME="shubhamrgandhi/qwen3-8b-full-sft-prm-r2egym-swebench-instructions-k5-opus-distill-32k-lr5e6-multiturn"
            PRM_PORT=8071
            ;;
        qwen25coder7b)
    ...
    python3 - "$CONFIG" "$TEMP_CONFIG" \
        "$AGENT_API_BASE" "$PRM_MODEL_NAME" "$PRM_API_BASE" "" "openai" "$DISABLE_THINKING" "$STEP_LIMIT" "$PRM_DEDUP" "$PRM_DEDUP_THRESHOLD" "$PREFIX_DIR" <<'PYEOF'
import yaml, sys

base_config, temp_config, agent_api_base, prm_model_name, prm_api_base, prm_api_key, prm_provider, disable_thinking, step_limit, prm_dedup, prm_dedup_threshold, prefix_dir = sys.argv[1:]
...
# Override agent model api_base — only if non-empty (Bedrock etc. should keep its own)
if agent_api_base:
    cfg['model']['model_kwargs']['api_base'] = agent_api_base
...
echo "Agent model: ${AGENT_MODEL} @ ${AGENT_API_BASE:-<provider default>}"
...
[ -n "$PREFIX_DIR" ] && echo "Prefix dir:  ${PREFIX_DIR}"
echo "Config:      ${CONFIG}"
echo "Output:      ${OUTPUT}"
