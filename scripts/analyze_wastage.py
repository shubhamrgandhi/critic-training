...
Inputs:
  ...
  --setting    : setting prefix for model directories (default: edit_obs_dev)
...
def plot_grouped_comparison(model_aggregates, key_type, outfile):
    ...
    percentage_matrix = np.zeros((n_models, n_categories))
    for i in range(n_models):
        for j in range(n_categories):
            total = immediate_matrix[i, j] + snowball_matrix[i, j] + non_wasted_matrix[i, j]
            if total > 0:
                percentage_matrix[i, j] = ((immediate_matrix[i, j] + snowball_matrix[i, j]) / total) * 100
    ...
    for i, model_name in enumerate(model_names):
        ...
        # Add percentage labels on top of bars ONLY for action_type plot
        if key_type == "action_type":
            for j in range(n_categories):
                total_height = (immediate_matrix[i, j] + snowball_matrix[i, j] + non_wasted_matrix[i, j]) / 1e6
                if total_height > 0 and percentage_matrix[i, j] > 0:
                    # Alternate label positions slightly to reduce overlap
                    y_offset = 3 if i % 2 == 0 else 6
                    
                    # Position label at top of bar with white background for visibility
                    ax.annotate(f'{percentage_matrix[i, j]:.1f}%',
                               xy=(x[j] + offset, total_height),
                               xytext=(0, y_offset),
...
def main():
    ...
    ap.add_argument(
        "--setting", 
        default="edit_obs_dev",
        type=str, 
        help="Setting prefix for model directories (e.g., edit_obs_dev)"
    )
    ap.add_argument(
        "--outdir", 
        default=None, 
    ...
    for model_name, _ in sorted_model_dirs:
        ...
        if display_name != model_name:
            print(f"  - {display_name} (from {model_name})")
        else:
            print(f"  - {model_name}")
    print()
