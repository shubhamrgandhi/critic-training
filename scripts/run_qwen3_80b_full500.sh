#!/bin/bash
#SBATCH --job-name=qwen3_80b_full500
#SBATCH --output=sbatch_logs/qwen3_80b_full500_%j.out
#SBATCH --error=sbatch_logs/qwen3_80b_full500_%j.err
...
#
# Submit:
#   sbatch scripts/run_qwen3_80b_full500.sh
#
# Attach:
#   ./connect_job.sh <jobid> qwen3_80b_full
...
mini-extra swebench \\
  --config mini-swe-agent/configs/swebench_singularity_edit_obs_final_only_0_qwen3-80b_max150.yaml \\
...
rc=\$?
echo
echo "=== qwen3_80b full500 done at \$(date) (rc=\$rc) ==="
...
tmux new-session -d -s qwen3_80b_full "$INNER_SCRIPT"

echo "=== Launched tmux session 'qwen3_80b_full' on $(hostname) ==="
echo "Inner script: $INNER_SCRIPT"
echo "To attach:    ./connect_job.sh ${SLURM_JOB_ID:-<jobid>} qwen3_80b_full"
