...
# run_prm_max150.sh
#
# Wrapper around run_prm.sh logic with configurable step limit.
...
#
# Usage: same as run_prm.sh
#   ./run_prm_max150.sh <setting> <prm_interval> <run> <agent_model> [options]
...
if [ -z "$SETTING" ] || [ -z "$PRM_INTERVAL" ] || [ -z "$RUN" ] || [ -z "$AGENT_MODEL" ]; then
    echo "Usage: ./run_prm_max150.sh <setting> <prm_interval> <run> <agent_model> [options]"
    exit 1
fi
