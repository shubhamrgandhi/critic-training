...
* `duplicate_read`: Reading the same file/content without intermediate external modifications. This also includes viewing specific parts of a file if that part has been read before even if the agent explicitly made changes to the file. Do not count duplicate reads if the file contents change as a result of executing scripts or commands. This also includes re-reading overlapping parts of the same file without any modifications having been made.
...
* `premature_validation`: Testing/building before implementation is complete
...
* `ignored_error_message`: Not adapting based on error output
* `circular_debugging`: Repeating same debug cycle without progress
* `abandoned_context`: Starting fresh instead of building on partial progress
...
def process_trajectories(results_dir: Path, output_dir: Path, limit: Optional[int] = None, 
                        model: str = "gpt-5-mini", api_key: str = None, delay: float = 0.5):
    """Process all trajectories in the results directory."""
    ...
    # --- Agent token wastage accumulators (from redundant steps) ---
    ...
    with Progress(
        ...
        for i, traj_file in enumerate(trajectory_files):
            progress.update(task, description=f"Processing {traj_file.name}")
            ...
            if not steps:
                rich.print(f"[yellow]No steps found in {traj_file.name}[/yellow]")
                continue
            ...
            # Map step_number -> agent usage for quick lookup
            usage_by_step = {
            ...
            for step_report in (judge_response.get("steps") or []):
                ...
                nxt = usage_by_step.get(sn + 1, {})  # might be empty on last step
                ...
                # Number of steps affected by this redundant step (current + all subsequent)
                num_subsequent_steps = total_steps - sn
                
                # Snowball wastage:
                # 1. Completion tokens from current step (wasted immediately)
                # 2. Context increase propagated through all subsequent steps
                ...
                total_agent_prompt_wasted += prompt_wasted
                total_agent_completion_wasted += completion_wasted
                total_agent_tokens_wasted += tokens_wasted
...
def main():
    parser = argparse.ArgumentParser(description="Run LLM judge on mini SWE-agent trajectories")
    
    parser.add_argument(
        "--results-dir",
        type=str,
        default="/usr0/home/srgandhi/tool-overuse/results_val",
    ...
    # Run analysis
    rich.print("[green]Starting mini SWE-agent trajectory analysis...[/green]")
