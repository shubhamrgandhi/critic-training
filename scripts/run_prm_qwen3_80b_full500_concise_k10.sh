#!/bin/bash
#SBATCH --job-name=prm_q380b_concise_k10_full500
#SBATCH --output=sbatch_logs/prm_q380b_concise_k10_full500_%j.out
#SBATCH --error=sbatch_logs/prm_q380b_concise_k10_full500_%j.err
...
# the existing 0_qwen3-80b base run. Mirror of the cwm reference run at:
...
#
# Submit:
#   sbatch scripts/run_prm_qwen3_80b_full500_concise_k10.sh
#
# Attach:
#   ./connect_job.sh <jobid> prm_q380b_concise_k10_full
...
echo "=== qwen3-80b agent + concise k=10 PRM (claude-opus-4-6) full 500 ==="
...
INNER_SCRIPT="/home/srgandhi/tool-overuse/sbatch_logs/prm_q380b_concise_k10_full500_${SLURM_JOB_ID:-manual}_inner.sh"
...
./run_prm_max150_mini.sh prm_issue_res_instructions 10 0 qwen3-80b \
  --no-agent-api-base \
  --workers 20 \
...
rc=$?
echo
echo "=== prm_q380b_concise_k10_full500 done at $(date) (rc=$rc) ==="
...
tmux new-session -d -s prm_q380b_concise_k10_full "$INNER_SCRIPT"

echo "=== Launched tmux session 'prm_q380b_concise_k10_full' on $(hostname) ==="
echo "Inner script: $INNER_SCRIPT"
echo "To attach:    ./connect_job.sh ${SLURM_JOB_ID:-<jobid>} prm_q380b_concise_k10_full"
