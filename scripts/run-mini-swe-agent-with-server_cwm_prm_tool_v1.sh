...
CONFIG="${CONFIGS_DIR}/swebench_singularity_edit_obs_final_only_prm_tool_v1_k${PRM_INTERVAL}_0_${MODEL_NAME}.yaml"
RESULTS_DIR="/home/srgandhi/tool-overuse/results_singularity"
OUTPUT="${RESULTS_DIR}/singularity_edit_obs_final_only_prm_tool_v1_k${PRM_INTERVAL}_0_${MODEL_NAME}_prm_claude-opus-4-6"
...
if [ ! -f "$CONFIG" ]; then
    ...
    ls -1 "${CONFIGS_DIR}/swebench_singularity_edit_obs_final_only_prm_tool_v1_k*_0_${MODEL_NAME}.yaml" 2>/dev/null || echo "  (none found)"
    exit 1
fi
...
AGENT_LOG_FILE="../agent_logs/agent_prm_tool_v1_k${PRM_INTERVAL}_${MODEL_NAME}_${SLURM_JOB_ID}_$(date +%Y%m%d_%H%M%S).log"
