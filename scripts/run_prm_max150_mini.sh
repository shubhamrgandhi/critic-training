#!/bin/bash
# run_prm_max150_mini.sh
#
# Same as run_prm_max150.sh but defaults to MariusHobbhahn/swe-bench-verified-mini
# subset with no slice limit.
#
# Usage: same as run_prm_max150.sh
#   ./run_prm_max150_mini.sh <setting> <prm_interval> <run> <agent_model> [options]
...
if [ -z "$SETTING" ] || [ -z "$PRM_INTERVAL" ] || [ -z "$RUN" ] || [ -z "$AGENT_MODEL" ]; then
    echo "Usage: ./run_prm_max150_mini.sh <setting> <prm_interval> <run> <agent_model> [options]"
    exit 1
fi
