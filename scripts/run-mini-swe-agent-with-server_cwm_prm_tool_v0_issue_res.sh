...
# PRM variant:  prm_tool_v0_issue_res (tool overuse v0 + issue resolution)
#
# Usage:
#   sbatch scripts/run-mini-swe-agent-with-server_cwm_prm_tool_v0_issue_res.sh
#   sbatch scripts/run-mini-swe-agent-with-server_cwm_prm_tool_v0_issue_res.sh --prm-interval 3
#   sbatch scripts/run-mini-swe-agent-with-server_cwm_prm_tool_v0_issue_res.sh --prm-interval 10 --slice 10:20
# =============================================================================
...
CONFIG="${CONFIGS_DIR}/swebench_singularity_edit_obs_final_only_prm_tool_v0_issue_res_k${PRM_INTERVAL}_0_${MODEL_NAME}.yaml"
RESULTS_DIR="/home/srgandhi/tool-overuse/results_singularity"
OUTPUT="${RESULTS_DIR}/singularity_edit_obs_final_only_prm_tool_v0_issue_res_k${PRM_INTERVAL}_0_${MODEL_NAME}_prm_claude-opus-4-6"

echo "=============================================="
echo "CWM + Claude Opus PRM (tool_v0 + issue_res)"
echo "=============================================="
...
if [ ! -f "$CONFIG" ]; then
    ...
    ls -1 "${CONFIGS_DIR}/swebench_singularity_edit_obs_final_only_prm_tool_v0_issue_res_k*_0_${MODEL_NAME}.yaml" 2>/dev/null || echo "  (none found)"
    exit 1
fi
...
AGENT_LOG_FILE="../agent_logs/agent_prm_tool_v0_issue_res_k${PRM_INTERVAL}_${MODEL_NAME}_${SLURM_JOB_ID}_$(date +%Y%m%d_%H%M%S).log"

echo ""
echo "----------------------------------------------"
echo "Experiment: PRM tool_v0_issue_res k=${PRM_INTERVAL} with ${MODEL_NAME}"
echo "Config:     $CONFIG"
echo "Output:     $OUTPUT"
