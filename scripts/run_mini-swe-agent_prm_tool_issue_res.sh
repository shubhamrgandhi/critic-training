#!/bin/bash
# run_mini-swe-agent_prm_tool_issue_res.sh
#
# Lightweight script for running CWM + PRM (combined tool overuse + issue
# resolution v2 — improved) when vLLM is already serving elsewhere.
# No server startup -- just the agent run.
#
# Usage:
#   ./run_mini-swe-agent_prm_tool_issue_res.sh <prm_interval> <run> <model>
#   ./run_mini-swe-agent_prm_tool_issue_res.sh 5 0 cwm
#   ./run_mini-swe-agent_prm_tool_issue_res.sh 3 0 cwm --slice 10:20
#   ./run_mini-swe-agent_prm_tool_issue_res.sh 5 0 cwm --workers 4 --slice :10
#   ./run_mini-swe-agent_prm_tool_issue_res.sh 5 0 cwm --api-base babel-s5-24:8070
...
if [ -z "$PRM_INTERVAL" ] || [ -z "$RUN" ] || [ -z "$MODEL" ]; then
    echo "Usage: ./run_mini-swe-agent_prm_tool_issue_res.sh <prm_interval> <run> <model> [extra args]"
    echo "Example: ./run_mini-swe-agent_prm_tool_issue_res.sh 5 0 cwm"
    echo "Example: ./run_mini-swe-agent_prm_tool_issue_res.sh 3 0 cwm --slice :10 --workers 4"
    exit 1
fi
...
SETTING="singularity_edit_obs_final_only_prm_tool_issue_res_k${PRM_INTERVAL}"
