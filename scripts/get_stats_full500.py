#!/usr/bin/env python3
"""
Compute stats for the full SWE-Bench Verified set (500 instances).

Only includes runs that have all 500 instances completed.
...
Usage:
    python get_stats_full500.py
    python get_stats_full500.py --parent-dir results_singularity_max_150_steps
"""
import argparse
import json
import re
...
MODEL_PRICING = {
    ...
    # Self-hosted models (from litellm_model_registry.json)
...
# (setting, agent_model, prm_label, dirname)
RUNS = [
    ("base", "CWM", "-",
     "singularity_edit_obs_final_only_0_cwm"),
    ("prm_issue_res_k5", "CWM", "Qwen3-8b",
     "singularity_edit_obs_final_only_prm_issue_res_k5_0_cwm_prm_qwen3-8b"),
    ("prm_issue_res_k5", "CWM", "Qwen3-8b + SFT + noisy",
     "singularity_edit_obs_final_only_prm_issue_res_k5_0_cwm_prm_qwen3-8b-full-sft-prm-opus-distill-32k-lr5e6"),
    ("prm_issue_res_k5", "CWM", "Qwen3-8b + SFT + clean",
     "singularity_edit_obs_final_only_prm_issue_res_k5_0_cwm_prm_qwen3-8b-full-sft-prm-opus-distill-32k-lr5e6_clean"),
    ("prm_issue_res_k5", "CWM", "Qwen3-8b + RS-SFT + clean",
...
def main():
    ...
    parser.add_argument("--parent-dir", type=str,
                        default=str(Path(__file__).resolve().parent.parent / "results_singularity_max_150_steps"))
    ...
    all_stats = []
    for setting, agent_model, prm_label, dirname in RUNS:
        run_dir = parent / dirname
        print(f"  {setting} / {agent_model} / {prm_label} ... ", file=sys.stderr, end="", flush=True)
        if not run_dir.exists():
            print("NOT FOUND", file=sys.stderr)
            all_stats.append((setting, agent_model, prm_label, None))
            continue
        ...
        all_stats.append((setting, agent_model, prm_label, stats))
    ...
    stats_rows = []
    for (setting, agent_model, prm_label, stats), (_, _, _, dirname) in zip(
            all_stats, RUNS):
        if stats is None:
            stats_rows.append([setting, agent_model, prm_label] + ["N/A"] * 7)
            continue
        stats_rows.append([
            setting, agent_model, prm_label,
            str(stats["resolved"]),
    ...
    exit_headers = ["Setting", "Agent Model", "PRM Model"] + STATUS_LABELS + ["Other", "Total"]

    exit_rows = []
    for (setting, agent_model, prm_label, stats), (_, _, _, dirname) in zip(
            all_stats, RUNS):
        if stats is None:
            exit_rows.append([setting, agent_model, prm_label] + ["N/A"] * 5)
            continue
