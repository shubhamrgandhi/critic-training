...
export DATA_DIR="${DATA_DIR:-$SCRIPT_DIR/prm_sft_data_opus_distill_full_feedback_history_32k_rejection-sample}"
export OUTPUT_DIR="${OUTPUT_DIR:-$SAVEDIR/qwen3-8b-full-sft-prm-opus-distill-32k-lr5e6_rejection-sample_think}"
...
echo "=== Qwen3-8B Full SFT for PRM — THINK mode (FSDP, 8x L40S) ==="
echo "Training data: $DATA_DIR/"
echo "Output: $OUTPUT_DIR"
echo "Template: qwen3 + enable_thinking: false (fast-thinking — preserves native reasoning)"
echo ""

cd "$LLAMAFACTORY_DIR"

# Resolve template YAML with envsubst
RESOLVED_YAML=$(mktemp /tmp/prm_full_sft_l40s_think_config_XXXXXX.yaml)
envsubst < "$SCRIPT_DIR/qwen3_8b_prm_full_sft_l40s_train_think.yaml" > "$RESOLVED_YAML"
...
echo "Serve with: vllm serve $OUTPUT_DIR --enable-reasoning --reasoning-parser deepseek_r1"
