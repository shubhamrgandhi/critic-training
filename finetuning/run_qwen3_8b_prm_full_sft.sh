...
export OUTPUT_DIR="$SAVEDIR/qwen3-8b-full-sft-prm-opus-distill"
mkdir -p "$LOGDIR" "$OUTPUT_DIR"

echo "=== Qwen3-8B Full SFT for PRM (FSDP, 8x A100) ==="
...
RESOLVED_YAML=$(mktemp /tmp/prm_full_sft_config_XXXXXX.yaml)
envsubst < "$SCRIPT_DIR/qwen3_8b_prm_full_sft_train.yaml" > "$RESOLVED_YAML"
trap "rm -f $RESOLVED_YAML" EXIT

# Train with accelerate + FSDP
# cpu_ram_efficient_loading ensures only rank 0 loads the model, avoiding CPU OOM
echo "=== Training ==="
accelerate launch \
    --config_file "$SCRIPT_DIR/fsdp_full_sft_config.yaml" \
    src/train.py "$RESOLVED_YAML" \
    2>&1 | tee "$LOGDIR/qwen3_8b_prm_full_sft_train.log"
